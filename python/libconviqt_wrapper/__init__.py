...
if available:
    ...
    class Beam(object):
        ...
        def __del__(self):
            try:
                ...
                print(
                    'WARNING: failed to free conviqt beam: "{}"'.format(e), flush=True
                )
    ...
    _conviqt.conviqt_detector_get_epsilon.argtypes = [
        ct.c_void_p,
    ...
    class Detector(object):
        ...
        def __del__(self):
            try:
                ...
                print(
                    'WARNING: failed to free conviqt detector: "{}"'.format(e),
                    flush=True,
                )
    ...
    class Pointing(object):
        ...
        def __del__(self):
            try:
                ...
                print(
                    'WARNING: failed to free conviqt pointing: "{}"'.format(e),
                    flush=True,
                )
        ...
        def alloc(self, nrow):
            ...
            if err != 0:
                raise RuntimeError(
                    "Failed to allocate {} rows to pointing".format(nrow)
                )
    ...
    class Convolver(object):
        ...
        def __init__(
            self, sky, beam, detector, pol, lmax, mmax, order, verbosity, comm
        ):
        ...
        def __del__(self):
            try:
                ...
                print(
                    'WARNING: failed to free conviqt convolver: "{}"'.format(e),
                    flush=True,
                )

        def convolve(self, pointing, calibrate=False):
            err = _conviqt.conviqt_convolver_convolve(
                self._conv, pointing._pnt, calibrate
            )
            if err != 0:
