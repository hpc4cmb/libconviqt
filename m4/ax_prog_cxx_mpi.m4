...
#
#  Revisions:
#    2016-05-20 : The macro now overrides CXX with MPICXX
...
AC_DEFUN([_AX_PROG_CXX_MPI], [
  ...
  if test x"$_ax_prog_cxx_mpi_mpi_wanted" = xyes; then
    if test -z "$CXX" && test -n "$MPICXX"; then
      ...
      AC_CHECK_TOOLS([MPICXX], [mpic++ mpicxx mpiCC sxmpic++ hcp mpxlC_r mpxlC mpixlcxx_r mpixlcxx mpg++ mpc++ mpCC cmpic++ mpiFCC CCicpc pgCC pathCC sxc++ xlC_r xlC bgxlC_r bgxlC openCC sunCC crayCC g++ c++ gpp aCC CC cxx cc++ cl.exe FCC KCC RCC],[not-found])
    fi
    AC_SUBST([CXX],[$MPICXX])
  fi
  AC_PROG_CXX
