...
template<typename T> void create_alm_pol
  ...
  for (int l=0; l<=lmax; ++l)
    ...
    if (powspec.tt(l) != 0)
      ...
      if (rms_g2 <= 0)
        {
          levels_assert (::abs(rms_g2) <= 1e-8*::abs(powspec.gg(l)),
...
template<typename T> void extract_crosspowspec
  ...
  {
  levels_assert (alm1.conformable(alm2), "a_lm are not conformable");
...
template<typename T> void extract_crosspowspec
  (const Alm<xcomplex<T> > &almT1,
   ...
   PowSpec &powspec)
  {
  levels_assert (almT1.conformable(almG1) && almT1.conformable(almC1) &&
...
template<typename T> void rotate_alm (Alm<xcomplex<T> > &alm,
  ...
  {
  levels_assert (alm.Lmax()==alm.Mmax(),
...
template<typename T> void rotate_alm (Alm<xcomplex<T> > &almT,
  ...
  double psi, double theta, double phi)
  {
  levels_assert (almT.Lmax()==almT.Mmax(),
    "rotate_alm: lmax must be equal to mmax");
  levels_assert (almG.conformable(almT) && almC.conformable(almT),
