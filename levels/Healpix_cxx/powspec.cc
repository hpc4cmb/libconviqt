...
void PowSpec::assertArraySizes() const
  {
  levels_assert((num_specs==1) || (num_specs==4) || (num_specs==6),
    "incorrect number of spectral components");
  if (num_specs==1)
    levels_assert(multiequal(tsize(0),gg_.size(),cc_.size(),tg_.size(),
      tc_.size(),gc_.size()), "incorrect array sizes");
  if (num_specs==4)
    {
    levels_assert(multiequal(tt_.size(),gg_.size(),cc_.size(),tg_.size()),
      "incorrect array sizes");
    levels_assert(multiequal(tsize(0),tc_.size(),gc_.size()),
      "incorrect array sizes");
    }
  if (num_specs==6)
    levels_assert(multiequal(tt_.size(),gg_.size(),cc_.size(),tg_.size(),
...
void PowSpec::smoothWithGauss (double fwhm)
  {
  levels_assert (num_specs<=4, "not yet implemented for num_specs>4");
