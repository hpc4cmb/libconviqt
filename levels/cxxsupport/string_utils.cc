...
string intToString(int64 x, tsize width)
  ...
  levels_assert(res.size()==width,"number too large");
  return trim(res);
  }
...
void end_stringToData (const string &x, const char *tn, istringstream &strstrm)
  ...
  levels_assert (strstrm,error);
  string rest;
  strstrm >> rest;
//  rest=trim(rest);
  levels_assert (rest.length()==0,error);
  }
...
template<> void stringToData (const string &x, bool &value)
  ...
  levels_fail("conversion error in stringToData<bool>(\""+x+"\")");
  }
...
void parse_file (const string &filename, map<string,string> &dict)
  ...
  levels_assert (inp,"Could not open parameter file '"+filename+"'.");
  while (inp)
    {
    string line;
    getline(inp, line);
...
void parse_cmdline_classic (int argc, const char **argv,
  ...
  {
  dict.clear();
  levels_assert(tsize(argc)>leading_args.size(),"not enough arguments");
  ...
  while (curarg<argc)
    ...
    levels_assert(isParam(param),"unrecognized command line format");
...
template<typename T> void split (istream &stream, vector<T> &list)
  ...
  while (stream)
    ...
    string word;
    stream >> word;
    levels_assert (stream||stream.eof(),
...
void parse_words_from_file (const string &filename, vector<string> &words)
  ...
  words.clear();
  ifstream inp(filename.c_str());
  levels_assert (inp,"Could not open file '"+filename+"'.");
  while (inp)
    {
    string word;
    inp>>word;
