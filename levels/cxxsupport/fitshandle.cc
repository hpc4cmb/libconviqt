...
#include <vector>
#include "fitsio.h"
#include "ls_fitshandle.h"
#include "ls_string_utils.h"
...
int type2bitpix (PDT type)
  ...
  switch (type)
    {
    case LEVELS_FLOAT32: return FLOAT_IMG;
    case LEVELS_FLOAT64: return DOUBLE_IMG;
    default: levels_fail ("unsupported component type");
    }
  }
...
PDT ftc2type (int ftc)
  ...
  switch (ftc)
    {
    case TLOGICAL : return LEVELS_BOOL;
    case TBYTE    : return LEVELS_INT8;
    case TSHORT   : return LEVELS_INT16;
    case TINT     :
    case TINT32BIT: return LEVELS_INT32;
    case TLONGLONG: return LEVELS_INT64;
    case TFLOAT   : return LEVELS_FLOAT32;
    case TDOUBLE  : return LEVELS_FLOAT64;
    case TSTRING  : return LEVELS_STRING;
    default: levels_fail ("unsupported component type");
    }
  }
...
int type2ftc (PDT type)
  ...
  switch (type)
    {
    case LEVELS_BOOL   : return TLOGICAL;
    case LEVELS_INT8   :
    case LEVELS_UINT8  : return TBYTE;
    case LEVELS_INT16  : return TSHORT;
    case LEVELS_INT32  : return TINT;
    case LEVELS_INT64  : return TLONGLONG;
    case LEVELS_FLOAT32: return TFLOAT;
    case LEVELS_FLOAT64: return TDOUBLE;
    case LEVELS_STRING : return TSTRING;
    default: levels_fail ("unsupported component type");
    }
  }
...
const char *type2fitschar (PDT type)
  ...
  switch (type)
    {
    case LEVELS_BOOL   : return "L";
    case LEVELS_FLOAT32: return "E";
    case LEVELS_FLOAT64: return "D";
    case LEVELS_INT8   :
    case LEVELS_UINT8  : return "B";
    case LEVELS_INT16  : return "I";
    case LEVELS_INT32  : return "J";
    case LEVELS_INT64  : return "K";
    case LEVELS_STRING : return "A";
    default:
      levels_fail(string("unknown data type ")+type2string(type));
    }
  }
...
const char *type2asciiform (PDT type)
  ...
  switch (type)
    {
    case LEVELS_FLOAT32: return "E14.7";
    case LEVELS_FLOAT64: return "D23.15";
    case LEVELS_UINT8  : return "I3";
    case LEVELS_INT8   : return "I4";
    case LEVELS_INT16  : return "I6";
    case LEVELS_INT32  : return "I11";
    case LEVELS_INT64  : return "I22";
    default:
      levels_fail(string("unknown data type ")+type2string(type));
    }
  }
...
fitscolumn::fitscolumn()
  : repcount_(0), type_(LEVELS_INVALID) {}
...
void fitshandle::init_data()
  ...
  switch(hdutype_)
    ...
    case BINARY_TBL:
      init_bintab(); break;
    default:
      levels_fail("init_data(): unsupported HDU type"); break;
    }
  }
...
void fitshandle::read_col (int colnum, void *data, int64 ndata, PDT type,
  int64 offset) const
  {
  levels_assert(table_hdu(colnum),"incorrect FITS table access");
  int64 repc = columns_[colnum-1].repcount();
  levels_assert (ndata<=(repc*nrows_-offset),"read_column(): array too large");
...
void fitshandle::getKeyHelper(const string &name) const
  ...
  if (status==KEY_NO_EXIST)
    ...
    fits_clear_errmsg();
    status=0;
    levels_fail("fitshandle::get_key(): key '"+name+"' not found");
    }
  check_errors();
  }
...
string fitshandle::fileName() const
  {
  levels_assert(connected(),"handle not connected to a file");
...
void fitshandle::insert_asctab (const vector<fitscolumn> &cols,
  ...
  for (long m=0; m<ncol; ++m)
    ...
    if (cols[m].type()!=TSTRING)
      {
      levels_assert (cols[m].repcount()==1,"bad repcount for ASCII table");
...
const vector<int64> &fitshandle::axes() const
  {
  levels_assert(image_hdu(),"not connected to an image");
  return axes_;
  }
const string &fitshandle::colname(int i) const
  {
  levels_assert(table_hdu(i),"incorrect FITS table access");
...
int fitshandle::ncols() const
  {
  levels_assert(table_hdu(1),"incorrect FITS table access");
...
int64 fitshandle::nelems(int i) const
  {
  levels_assert(table_hdu(i),"incorrect FITS table access");
  if (columns_[i-1].type()==LEVELS_STRING) return nrows_;
...
int64 fitshandle::efficientChunkSize(int i) const
  ...
  levels_assert(res>=1,"bad recommended FITS chunk size");
  check_errors();
...
void fitshandle::set_key_void (const string &key, const void *value,
  ...
  switch (type)
    ...
    default:
      levels_fail ("unsupported data type in set_key_void()");
    }
  check_errors();
  }
...
void fitshandle::get_key_void (const string &name, void *value, PDT type) const
  ...
  switch (type)
    ...
    default:
      levels_fail ("unsupported data type in get_key_void()");
    }
  check_errors();
  }
...
void fitshandle::read_column_raw_void
  ...
  switch (type)
    ...
    case LEVELS_STRING:
      ...
      levels_assert(table_hdu(colnum),"incorrect FITS table access");
      levels_assert (num<=(nrows_-offset),
      ...
      levels_assert(dispwidth<=columns_[colnum-1].repcount(),"column too wide");
    ...
    default:
      levels_fail ("unsupported data type in read_column_raw_void()");
    }
  }
...
void fitshandle::write_column_raw_void
  ...
  switch (type)
    ...
    default:
      levels_fail ("unsupported data type in write_column_raw_void()");
    }
  }
...
void fitshandle::write_image2D_void (const void *data, PDT type, tsize s1,
  ...
  {
  levels_assert(image_hdu(),"not connected to an image");
  levels_assert (axes_.size()==2, "wrong number of dimensions");
  levels_assert (axes_[0]==int64(s1), "wrong size of dimension 1");
  levels_assert (axes_[1]==int64(s2), "wrong size of dimension 2");
...
template<typename T> void fitshandle::read_image (arr3<T> &data) const
  {
  levels_assert(image_hdu(),"not connected to an image");
  levels_assert (axes_.size()==3, "wrong number of dimensions");
  data.alloc(safe_cast<tsize>(axes_[0]), safe_cast<tsize>(axes_[1]),
...
class cfitsio_checker
  ...
  public:
    cfitsio_checker()
      {
      float fitsversion;
      levels_assert(fits_get_version(&fitsversion),
